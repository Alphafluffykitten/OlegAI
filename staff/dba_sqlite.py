...
# abandoned
class OlegDBAdapter():
    """ DB adapter holds info about OlegDB structure and provides objects from DB to clients """
    ...
    def get_users(self):
        ...
        cols = self.get_column_names('users')
        ...
        users = []
        for r in rows:
            ukw = {colname:r[idx] for idx,colname in enumerate(cols)}
            users.append(User(**ukw))
        return users
    ...
    def get_posts(
        ...
        if have_content is not None:
            clauses.append(f'content_downloaded = {int(have_content)}')
    ...
    def get_post_ids(self,content_downloaded=None):
        """ gets list of all post ids from OlegDB """

        clauses = []
        if content_downloaded is not None:
            clauses.append(f'content_downloaded = {int(content_downloaded)}')
    ...
    def add_post(self, post: Post):
        ...
        if not posts:
            ...
            post = self.get_posts(tg_msg_id = post.tg_msg_id, tg_channel_id = post.tg_channel_id)
            if not post:
                raise Exception(f'[ OlegDBAdapter ]: Couldn\'t add new post to DB. tg_msg_id: {tg_msg_id}, tg_channel_id: {tg_channel_id}')
            post = post[0]

            return post

    def set_content_downloaded(self, post: Post):
        """ sets content_downloaded=1 """
        
        sql = f'''
            UPDATE posts
            SET content_downloaded = 1
            WHERE
                id = {post.id} 
            ORDER BY id
            LIMIT 1
    ...
    def get_user_reactions(self, user_id=None, internal_post_id=None, learned:bool=None, limit=10000, order = 'DESC'):
        ...
        if learned is not None:
            clauses.append(f'learned = {int(learned)}\n')
    ...
    def delete_post(self,post:Post):
        ...
        sql = f'DELETE FROM posts WHERE id = {post.id} LIMIT 1'
        self.db.push(sql)
    ...
    def get_channel(self, id = None, tg_channel_id = None, listening = True, name = None):
        ...
        if listening is not None:
            clauses.append(f'listening = {int(listening)}')
    ...
    def add_channel(self, channel):
        ...
        if not channels:
            sql = f'INSERT INTO channels (tg_channel_id, listening, name) VALUES ({channel.tg_channel_id},{channel.listening}, ?)'
        else:
            sql = f'''
            UPDATE channels
            SET
